...
int APIENTRY WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nCmdShow)
{
    ...
    //Configuracao de tela cheia.
    int windowWidth = GetSystemMetrics(SM_CXSCREEN);
    int windowHeigth = GetSystemMetrics(SM_CYSCREEN);
    long windowFull = WS_POPUP;

    //configuracao de janela.
    int windowWidthFix = 960;
    int windowHeigthFix = 540;
    long windowSmall = WS_OVERLAPPED | WS_SYSMENU;//WS_OVERLAPPEDWINDOW
    int windowPosX = windowWidth / 2 - windowWidthFix / 2;
    int windowPosY = windowHeigth / 2 - windowHeigthFix / 2;

    // Criando uma janela baseada na classe "BasicWindow" 
    hwnd = CreateWindowEx(NULL,
        "BasicWindow",                // classe da janela
        "Aplicação",                  // título da janela
        windowSmall,                  // estilo da janela
        windowPosX,                   // posição x inicial
        windowPosY,                   // posição y inicial
        windowWidthFix,               // largura inicial
        windowHeigthFix,              // altura inicial
    ...
    RECT winRect = { 0, 0, windowWidthFix, windowHeigthFix };
    ...
    windowPosX = (GetSystemMetrics(SM_CXSCREEN) / 2) -
        ((winRect.right - winRect.left) / 2);
    windowPosY = (GetSystemMetrics(SM_CYSCREEN) / 2) -
